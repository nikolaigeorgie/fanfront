...
export const joinQueue = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const totalMinutesAhead = (position - 1 + event.physicalLineThreshold) * event.slotDuration;
...
export const updateQueuePositions = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const totalMinutesAhead = (newPosition - 1 + event.physicalLineThreshold) * event.slotDuration;
